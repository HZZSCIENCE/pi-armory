...
export default function (pi: ExtensionAPI): void {
  ...
  function header(ctx: ExtensionContext) {
    ...
    container.addChild(new Text("  " + theme.fg("success", " ▶") + theme.fg("accent", " /armory") + theme.fg("muted", "  会话树预览 · Tree Preview") + theme.fg("dim", "  [默认开启]"), 1, 0));
    container.addChild(new Text("  " + theme.fg("dim", " ▷") + theme.fg("warning", " /plan") + theme.fg("muted", "    计划模式 · Plan Mode"), 1, 0));
    container.addChild(new Text("  " + theme.fg("success", " ▶") + theme.fg("accent", " /trash") + theme.fg("muted", "  回收站 · Recycle Bin") + theme.fg("dim", "  [默认开启]"), 1, 0));
